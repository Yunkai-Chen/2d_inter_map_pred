...
def ensure_TlTp_order(t: torch.Tensor, Tl: int, Tp: int) -> torch.Tensor:
    """
    Normalize model output shapes to:
      - 3D: [B, Tl, Tp]
      - 4D: [B, Tl, Tp, C]
    """
    if t.ndim == 3:
...
# ---------------------------
# Distance binning helpers (for multi-class distance)
# ---------------------------
...
def bin_centers_from_edges(edges: torch.Tensor) -> torch.Tensor:
    e = edges
    centers = 0.5 * (e[:-1] + e[1:])
    centers[-1] = e[-2] + 4.0  # representative for [last, +inf)
    return centers
...
def distances_to_soft_targets(dist: torch.Tensor,
    ...
    if smooth_alpha > 0:
        ...
        Q.scatter_(dim=-1, index=left.unsqueeze(-1),
                src=Q.new_full(left.unsqueeze(-1).shape, float(w)))
        Q.scatter_(dim=-1, index=right.unsqueeze(-1),
                src=Q.new_full(right.unsqueeze(-1).shape, float(w)))
...
class BinnedDistanceCriterion(torch.nn.Module):
    """
    Cross-entropy with soft targets for distance bins (+ optional focal & class weights).
    Aggregates over valid pair_mask.
    """
    def __init__(self,
                 edges: torch.Tensor,
...
# ---------------------------
@torch.no_grad()
def evaluate(model, criterion, loader, device, is_bins: bool = False):
    model.eval()
...
def main():
    ...
    # model
    # after
    ap.add_argument("--head", choices=["bilinear","mlp","axial"], default="mlp")

    # add these axial-specific knobs near other model args
    ap.add_argument("--axial-c-pair",      type=int, default=128, help="pair-channel dim inside axial head")
    ap.add_argument("--axial-layers",      type=int, default=2,   help="# of axial transformer layers")
    ap.add_argument("--axial-heads",       type=int, default=4,   help="# of attention heads")
    ap.add_argument("--axial-ffn-hidden",  type=int, default=512, help="FFN hidden size per axial block")
    ...
    # scalar losses (contact/distance regression)
    ...
    ap.add_argument("--bin-smooth", type=float, default=0.2)
    ap.add_argument("--bin-focal-gamma", type=float, default=0.0)
    ...
    # NEW: LR schedule (warmup + cosine)
    ap.add_argument("--lr-warmup-steps", type=int, default=1000)
    ap.add_argument("--lr-min-ratio", type=float, default=0.1)  # cosine tail ratio (0~1)
    ...
    if args.distance_bins:
        if args.gt_as_contact:
            raise ValueError("distance-bins requires --gt-as-distance.")
        if args.head != "mlp":
        ...
        cfg = PairModelConfig(
            ...
            dropout=args.dropout, chunk_ll=args.chunk_ll, score_activation=None,
            out_channels=C, temperature=max(1e-6, args.temperature)
        )
        ...
        # else: scalar/contact path (keep as is, just add axial fields)
        cfg = PairModelConfig(
            d_model=args.d_model,
            head=args.head,                      # now can be "axial"
            hidden=args.mlp_hidden,
            d_proj=args.d_proj,
            dropout=args.dropout,
            chunk_ll=args.chunk_ll,
            ...
            # NEW: pass-through for axial head
    ...
    for epoch in range(start_epoch, args.epochs + 1):
        ...
        for step, batch in enumerate(loader_train, start=1):
            for name, tens in [("prot_emb", batch["protein_emb"]), ("pep_emb", batch["peptide_emb"])]:
                ...
                if n_bad and is_main_process(): print(f"[warn] {name} non-finite count = {n_bad}")
            ...
            # grad clip + grad norm (monitor)
            total_grad = 0.0
            if args.grad_clip and args.grad_clip > 0:
                scaler.unscale_(opt)
                torch.nn.utils.clip_grad_norm_(model.parameters(), args.grad_clip)
            for p in (model.module if isinstance(model, DDP) else model).parameters():
                if p.grad is not None:
                    g = p.grad.detach()
                    total_grad += float(g.norm(2).item()**2)
            ...
            if is_main_process() and ((step % args.log_interval) == 0 or step == 1):
                log_line = f"[epoch {epoch} step {step}] loss={float(loss):.4f}"
                s = out["scores"]
                if s.ndim == 4:  # multi-class (bins)
                    with torch.no_grad():
                        probs = torch.softmax(s.float(), dim=-1)
                        ent = (-probs * (probs.clamp_min(1e-12)).log()).sum(dim=-1)  # [B,Tl,Tp]
                        ent_mean = float(ent.mean().item())
                        mn, mx = float(s.min().item()), float(s.max().item())
                        log_line += f" | logits[{mn:.1f},{mx:.1f}] H={ent_mean:.3f}"
                        if "mae" in loss_dict: log_line += f"  mae={loss_dict['mae']:.4f}"
                elif s.ndim == 3:  # scalar
                    mn, mx = float(s.min().item()), float(s.max().item())
                    log_line += f" | scores[{mn:.1f},{mx:.1f}]"
                try: log_line += f"  lr={opt.param_groups[0]['lr']:.2e}"
                except: pass
                log_line += f"  grad_norm={total_grad:.3f}"
                print(log_line)
            ...
            if is_main_process() and wandb is not None and wandb.run is not None:
                ...
                if "mae" in loss_dict: logdic["train/mae"] = float(loss_dict["mae"])
