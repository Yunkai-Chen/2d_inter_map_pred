...
class PairMLP(nn.Module):
    """
    使用对偶特征拼接 [p, l, |p-l|, p*l] -> MLP，直观但显存较大。
    """
    ...
    def forward(self, prot_emb: torch.Tensor, pep_emb: torch.Tensor,
        ...
        """
        返回 scores:[B, Lp, Ll]。可通过 chunk_ll 沿 Ll 方向分块降低显存。
        """
        ...
        if chunk_ll and chunk_ll < Ll:
            ...
            for j0 in range(0, Ll, chunk_ll):
                ...
                out = self.mlp(feats.reshape(B * Lp * (j1 - j0), -1)).reshape(B, Lp, j1 - j0, 1).squeeze(-1)
                scores[:, :, j0:j1] = out
            return scores
        else:
            feats = self._make_pair_feats(P, pep_emb[:, None, :, :])  # [B,Lp,Ll,4D]
            scores = self.mlp(feats.reshape(B * Lp * Ll, -1)).reshape(B, Lp, Ll, 1).squeeze(-1)
            return scores


class PairBilinear(nn.Module):
    """
    双线性 + 仿射打分：p_i^T W l_j + u^T p_i + v^T l_j + b
    只用 bmm/einsum，极省显存，适合大 Lp×Ll。
    """
...
class PairModelConfig:
    ...
    # softplus 常用于把分数映射为正距离；sigmoid 可当作概率。

class PairwiseModel(nn.Module):
    """
    输入：prot_emb [B,Lp,D], pep_emb [B,Ll,D], prot_mask [B,Lp], pep_mask [B,Ll]
    输出：scores [B,Lp,Ll]（未激活/或按配置激活），以及 pair_mask [B,Lp,Ll]
    """
    ...
    def forward(self, batch: Dict[str, Any]) -> Dict[str, torch.Tensor]:
        ...
        if isinstance(self.head, PairMLP):
            scores = self.head(P, L, chunk_ll=self.cfg.chunk_ll)   # [B,Lp,Ll]
        else:
            scores = self.head(P, L)                               # [B,Lp,Ll]
